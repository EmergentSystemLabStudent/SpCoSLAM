#coding:utf-8
#PRR評価用プログラム（範囲はデータセットから決まる版）+URによるMI効果のある重みづけ計算
...
import os.path
from Julius1best_gmm_PRR import *
#import time
...
def Evaluation2(trialname):
  ...
  fp = open(datafolder + trialname + '/' + trialname + '_EvaluationPRR_MI.csv', 'w')  
  ...
  fp_PRR = open(datafolder + trialname + '/' + trialname + '_meanEvaluationPRR_MI.csv', 'w')  
...
if __name__ == '__main__':
    ...
    if (("000t" in filename) or ("000b" in filename)):
      ...
      R = 1
    """
    for s in range(8,11): #１からstepの数字までのループ
