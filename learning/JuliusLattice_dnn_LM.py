# encoding: shift_jis
# Akira Taniguchi 2017/01/22-2017/02/20-2017/07/08
# 実行すれば、自動的に指定フォルダ内にある音声ファイルを読み込み、Juliusでラティス認識した結果を出力してくれる。
# 注意点：指定フォルダ名があっているか確認すること。(パーティクルごとにLMを持つバージョン)
# step:教示回数
# filename:ファイル名
import glob
import codecs
...
# Lattice認識
def RecogLattice( wavfile , step , filename , trialname, particle, pre):
    ...
    if (step == 0 or step == 1):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)
      ...
      p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-dnn.jconf -v " + datafolder + trialname + "/" + str(step-1) + "/WD" + str(particle)+".htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice -dnnconf " + Juliusfolder + "julius.dnnconf $*" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS  -confnet
      print "Read dic: " + str(step-1) + "/WD" + str(pre) + ".htkdic" , step
...
def Julius_lattice(step, filename, trialname, particle,pre):
    step = int(step)
    Makedir( filename + "/fst_gmm" )
    Makedir( filename + "/fst_gmm/" + str(particle) )
    Makedir( filename + "/out_gmm" )
    Makedir( filename + "/out_gmm/" + str(particle) )
    ...
    for f in files2:
        txtfstfile = filename + "/fst_gmm/" + str(particle)+ "/%03d.txt" % num
        print "count...", f , num

        # Lattice認識&保存
        graph = RecogLattice( f, step, filename, trialname ,particle,pre)
    ...
    # 単語辞書を作成
    f = codecs.open( filename + "/fst_gmm/" + str(particle)+ "/isyms.txt" , "w" , "sjis" )
    ...
    # バイナリ形式へコンパイル
    fList = open( filename + "/fst_gmm/" + str(particle)+ "/fstlist.txt" , "wb" )  # 改行コードがLFでないとダメなのでバイナリ出力で保存
    for i in range(num):
        print "now compile..." , filename + "/fst_gmm/" + str(particle)+ "/%03d.txt" % i
        
        # FSTコンパイル
        FSTCompile( filename + "/fst_gmm/" + str(particle)+ "/%03d.txt" % i , filename + "/fst_gmm/" + str(particle)+ "/isyms.txt" , filename + "/fst_gmm/" + str(particle)+ "/%03d" % i  ,trialname, trialname)
        
        # lattice lm用のリストファイルを作成
        fList.write( filename + "/fst_gmm/" + str(particle)+ "/%03d.fst" % i )
        fList.write( "\n" )
    fList.close()
