...
# Akira Taniguchi 2017/01/18-2017/02/02-2017/02/15-
##############################################
...
def ReadImageData2(trialname, datasetname, step, clocktime):
  ...
  #現在時刻の画像データを読み込む
  files = glob.glob(datasetfolder + datasetname + "CNN/*.csv")
  files.sort()
  files2 = [files[i][len(datasetfolder)+len(datasetname)+4:-4] for i in xrange(len(files))]
  #print files2
  #temptime = []
  #for f in files:
  #  m = re.match('[^\d]*(\d+).*$', f)
...
def ReadWordData(step, filename, particle):
      ...
      Otb = []
      #テキストファイルを読み込み
      for line in open( filename + '/out_gmm/' + str(particle) + '/samp.'+str(samps), 'r'):   ##*_samp.*を読み込む
...
def ParticleSearcher(trialname):
  ...
  return X_To, step, m_count, CT, IT, pre
...
if __name__ == '__main__':
    ...
    Xp,step,m_count,CT,IT,pre = ParticleSearcher(trialname)
    #step = 4
    ...
    for i in xrange(R):
      print "--------------------------------------------------"
      print "Particle:",i
      Julius_lattice(step,filename,trialname,i,pre[i])    ##音声認識、ラティス形式出力、opemFST形式へ変換###########
      ...
      latticelm_CMD = "latticelm -input fst -filelist "+ filename + "/fst_gmm/" +str(i)+ "/fstlist.txt -prefix " + filename + "/out_gmm/" +str(i)+ "/ -symbolfile " + filename + "/fst_gmm/" +str(i)+ "/isyms.txt -burnin " + str(burnin) + " -samps " + str(samps) + " -samprate " + str(samprate) + " -knownn " + str(knownn) + " -unkn " + str(unkn) + " -annealsteps " + str(annealsteps) + " -anneallength " + str(anneallength)
      
      p = os.popen( latticelm_CMD )   ##latticelm ###################
      #time.sleep(1.0) #sleep(秒指定)###################
      p.close()###########################
      latticelm_count = 0
      while (os.path.exists(filename + "/out_gmm/" +str(i)+ "/samp."+str(samps) ) != True):
              print filename + "/out_gmm/" +str(i)+ "/samp."+str(samps),"wait(10s)... or ERROR?"
              #p.close()
              latticelm_count += 1
