...
def RecogLattice( wavfile , step , filename , trialname, nbest):
    ...
    if (step == 0 or step == 0):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)####使用されないはず
      p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + lmfolder + lang_init + " -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -output " + str(N_best_number+1) ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS 
      ...
      p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + datafolder + trialname + "/" + str(step) + "/WDonly.htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -output " + str(N_best_number+1) ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS 
