...
# Akira Taniguchi 2017/01/18-2017/02/02-2017/02/10-
##############################################
...
def Learning(step, filename, particle, XT, ST, W_list, CT, IT, FT):
    ...
    if (step == 1):  #初期設定
      ...
      IT.append( it )
      #Cの場所概念にStのカウントを足す
      Nlg_c = [sum([np.array(ST[s])*(CT[s]==cclist[c]) for s in xrange(step)]) for c in xrange(Lp)] 
      Wc_temp = [(np.array(Nlg_c[c]) + beta0 ) / (sum(Nlg_c[c]) + G*beta0) for c in xrange(Lp)]
      W = [Wc_temp[c] for c in xrange(Lp)]
      
      #Cの場所概念にFtのカウントを足す
      Nle_c = [sum([np.array(FT[s])*(CT[s]==cclist[c]) for s in xrange(step)]) for c in xrange(Lp)] 
