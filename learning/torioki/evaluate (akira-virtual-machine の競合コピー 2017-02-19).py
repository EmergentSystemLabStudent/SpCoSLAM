#coding:utf-8
#TCDS2の評価用プログラムを流用、改造したSpCoSLAM用評価プログラム
#Akira Taniguchi 
#2017/02/13-
import sys
import os.path
...
###↓###ARI############################################ok
def ARI(step,CT,IT):  #Ct = []  推定された場所概念のindex  
  CtC = [] #正解の分類結果
  ItC = [] #正解の分類結果
...
###↓###場所の名前のPAR############################################
def Name_of_Place(cstep, trialname, THETA, particle):
    ...
    if (1):
        Xt = []
        #最終時点step=50での位置座標を読み込み
        p = ReadParticleData(step,trialname)
        ItC_cstep = [ItC[i] for i in xrange(cstep+1)]
        ic_cstep = collections.Counter(ItC_cstep)
        Ni = len(ic_cstep)  #cstepにおける教示した場所の種類数
        
        for i in xrange(cstep+1):
          Xt = [[p[i].x,p[i].y]]
...
def ReaditCtData(trialname, step, particle):
  CT,IT = [],[]
  if (step != 0):  #最初のステップ以外
    for line in open( datafolder + trialname + "/" + str(step) + "/particle" + str(particle) + ".csv" , 'r' ):
      itemList = line[:-1].split(',')
...
def Evaluation(trialname):
  ...
  ARIc_List =  [0.0 for j in xrange(step)]
  ARIi_List =  [0.0 for j in xrange(step)]
  PARs_List = [0.0 for j in xrange(step)]
  PARw_List = [0.0 for j in xrange(step)]
  MAX_Samp =  [0 for j in xrange(step)]
  
  #イテレーションごとに選ばれた学習結果の評価値をすべて保存するファイル
  fp = open(datafolder + trialname + '/' + trialname + '_Evaluation.csv', 'w')  
  fp.write('ARIc,ARIi,PARs,PARw,L,K\n')
  
  #各stepの平均を保存
  fp2 = open(datafolder + trialname + '/' + trialname + '_meanEvaluation.csv', 'w')  
  fp2.write('ARIc,ARIi,PARs,PARw,L,K\n')
  ...
  for s in xrange(step):
    ...
    CT,IT = ReaditCtData(trialname, s+1, MAX_Samp[s])
    ...
    #推定されたLとKの数を読み込み
    i = 0
    for line in open(datafolder + trialname + '/'+ str(s+1) + '/index' + str(MAX_Samp[s]) + '.csv', 'r'):   ##読み込む
        itemList = line[:-1].split(',')
        if (i == 0):
    ...
    if (0):
      ...
      i = 0
      #テキストファイルを読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_w_index_'+str(s+1) + "_" + str(particle) + '.csv', 'r'):   ##*_samp.100を順番に読み込む
        itemList = line[:-1].split(',')
      ...
      #####パラメータW、μ、Σ、φ、πを入力する#####
      Myu = [ np.array([[ 0 ],[ 0 ]]) for i in xrange(K) ]      #位置分布の平均(x,y)[K]
      S = [ np.array([ [0.0, 0.0],[0.0, 0.0] ]) for i in xrange(K) ]      #位置分布の共分散(2×2次元)[K]
      W = [ [beta0 for j in xrange(len(W_index))] for c in xrange(L) ]  #場所の名前(多項分布：W_index次元)[L]
      theta = [ [0.0 for j in xrange(len(DimImg))] for c in xrange(L) ] 
      ...
      i = 0
      ##Myuの読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_Myu_'+str(s+1) + "_" + str(particle) + '.csv', 'r'):
        itemList = line[:-1].split(',')
      ...
      i = 0
      ##Sの読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_S_'+str(s+1) + "_" + str(particle) + '.csv', 'r'):
        itemList = line[:-1].split(',')
      ...
      c = 0
      #テキストファイルを読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_phi_'+str(s+1) + "_" + str(particle) + '.csv', 'r'):
        itemList = line[:-1].split(',')
        #print c
        #W_index = W_index + [itemList]
        for i in xrange(len(itemList)):
      ...
      ##piの読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_pi_'+str(s+1) + "_" + str(particle) + '.csv', 'r'):
        itemList = line[:-1].split(',')
      ...
      c = 0
      #テキストファイルを読み込み
      for line in open('./data/' + trialname +'/' + trialname + '_W_' + str(s+1) + '_' + str(particle) + '.csv', 'r'):
        itemList = line[:-1].split(',')
        #print c
        #W_index = W_index + [itemList]
        for i in xrange(len(itemList)):
    ...
    print "ARI"
    ARIc_List[s],ARIi_List[s] = ARI(s+1,CT,IT)
    
    if(0):
      print "PAR_S"
      PARs_List[s] = PAR_sentence(s, trialname, particle)
      
      THETA = [W,W_index,Myu,S,pi,phi_l,theta]
      #NOP = []
      print "PAR_W"
      PARw_List[s] = Name_of_Place(s, trialname, THETA, particle)
    ...
    fp.write( str(ARIc_List[s])+','+ str(ARIi_List[s])+','+ str(PARs_List[s])+','+str(PARw_List[s])+','+str(L)+','+str(K) )
    fp.write('\n')
    
  meanARIc = sum(ARIc_List)/float(step)
  meanARIi = sum(ARIi_List)/float(step)
  meanPARs = sum(PARs_List)/float(step)
  meanPARw = sum(PARw_List)/float(step)
  
  fp2.write( str(meanARIc)+','+ str(meanARIi)+','+ str(meanPARs)+','+str(meanPARw) )
  fp2.write('\n')
