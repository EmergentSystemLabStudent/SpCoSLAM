...
proc = mp.cpu_count()-1   # CPU-1並列とする
...
def MakeTmpWavListFile( wavfile ,wavfile2 , trialname):
    Makedir( datafolder + trialname + "/" + "tmp" )
    Makedir( datafolder + trialname + "/" + "tmp/" + trialname + "_" + wavfile2 )
    fList = codecs.open( datafolder + trialname + "/" + "tmp/" + trialname + "_" + wavfile2 + "/list.txt" , "w" , "sjis" )
    fList.write( wavfile )
    fList.close()

# Lattice認識
def RecogLattice( wavfile , step , filename , trialname):
    wavfile2 = wavfile[46:-4]
    MakeTmpWavListFile( wavfile ,wavfile2 , trialname )
    
    if (step == 0 or step == 1):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)
      p = os.popen( Juliusfolder + "bin/julius -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + lmfolder + lang_init + " -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "_" + wavfile2 + "/list.txt -confnet -lattice" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS 
      ...
      p = os.popen( Juliusfolder + "bin/julius -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + datafolder + trialname + "/" + str(step-1) + "/WD.htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "_" + wavfile2 + "/list.txt -confnet -lattice" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS 
...
def heiretsu( num,filename,f,step,trialname):
        #num = mpc[0]
        #filename = mpc[1]
        #f = mpc[2]
        #step = mpc[3]
...
def Julius_lattice(step, filename, trialname):
    ...
    #pool = mp.Pool(proc)
    #graph = pool.map
    #graph = [mp.Process(target = SaveLattice, args = (RecogLattice( files2[i], step, filename, trialname ), filename + "/fst_gmm/"+str(i).zfill(3)+".txt")) for i in xrange(len(files2))] 
    graph = [mp.Process(target = heiretsu, args = ( i, filename, files2[i], step, trialname ) ) for i in xrange(len(files2))] 
    ...
    print "multiproccess done."
    for f in files2:
      wordDic.add(queue.get())
      print WordDic
