...
#c-od-i-n-:-ut-f---8
#!/usr/bin/env python
#時系列データのグラフ描画プログラム、各評価値のstep推移をグラフ化する。(なにかのために仮使用したやつ)
...
for d in xrange(len(DATA)):
  ...
  for s in range(N):
    ...
    #if (d == 0 or 2):
    #  data_name = 'TAMD2_sig'
    for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_meanEvaluation.csv', 'r'):
...
if (1):
  ...
  AAA.legend(loc='upper left',fontsize=12)#title='method',
  #sns.tsplot(A, ci=50)
  ...
  plt.ylabel(hyouka,fontsize=14)
  plt.xlabel("step",fontsize=14)
  
  print df2
  #df2.to_csv("./text"+ hyouka+"2.csv")
  ...
  #fig = AAA.get_figure()
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2.eps', dpi=300)#, transparent=True
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2.pdf', dpi=300)#, transparent=True
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2.png', dpi=300)#, transparent=True
  #h = h+1
