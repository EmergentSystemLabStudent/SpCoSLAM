...
###↓###NMI############################################ok
def NMI(step,CT,IT):  #Ct = []  推定された場所概念のindex  
  CtC = [] #正解の分類結果
  ItC = [] #正解の分類結果
...
def Evaluation(trialname):
  ...
  fp.write('ARIc,ARIi,NMIc,NMIi,PARs,PARw,L,K,TSEG,ESEG\n')
  
  #各stepの平均を保存
  fp2 = open(datafolder + trialname + '/' + trialname + '_meanEvaluation.csv', 'w')  
  fp2.write('ARIc,ARIi,NMIc,NMIi,PARs,PARw,L,K,TSEG,ESEG\n')
