...
# Reading particle data (ID,x,y,theta,weight,previousID)
def ReadParticleData(step, particle, trialname):
  p = []
  for line in open ( datafolder + trialname + "/"+ str(step) + "/particle" + str(particle) + ".csv", "r" ):
    itemList = line[:-1].split(',')
    p.append( Particle( int(itemList[0]), float(itemList[1]), float(itemList[2]), float(itemList[3]), float(itemList[4]), int(itemList[5])) )
...
def Evaluation(trialname):
  ...
  fp = open(datafolder + trialname + '/' + trialname + '_EvaluationWORD.csv', 'w')  
  fp.write('ARIc,ARIi,NMIc,NMIi,PARwMI,PARw,L,K,TSEG,ESEG\n')
  
  #各stepの平均を保存
  fp2 = open(datafolder + trialname + '/' + trialname + '_meanEvaluationWORD.csv', 'w')  
  ...
  i = 0
  #重みファイルを読み込み
  for line in open(datafolder + trialname + '/'+ str(endstep) + '/weights.csv', 'r'):   ##読み込む
        #itemList = line[:-1].split(',')
        if (i == 0):
          MAX_Samp = int(line)
        i += 1
  ...
  p = ReadParticleData(endstep, particle, trialname)
  
  #各stepごとの評価値を算出する
  for s in xrange(step):
    
    #if (s != 0):
    #  particle = p[endstep -s+1].pid
    ...
    if(1):
        ...
        #NOP = []
        print "PAR_W"
        PARw_List[s][r] = Name_of_Place(s+1, trialname, THETA, particle, L[s][particle], K[s][particle])
        
        print "PAR_W_MI"
        PARs_List[s][r] = Name_of_Place_MI(s+1, trialname, THETA, particle, L[s][r], K[s][r])
    ...
    fp2.write( str(ARIc_List[s][particle])+','+ str(ARIi_List[s][particle])+','+ str(NMIc_List[s][particle])+','+ str(NMIi_List[s][particle])+','+ str(PARs_List[s][particle])+','+str(PARw_List[s][particle])+','+str(L[s][particle])+','+str(K[s][particle])+','+str(TSEG[s][particle])+','+str(ESEG[s][particle]) )
