...
def RecogLattice( wavfile , step , filename , trialname):
    ...
    if (step == 0 or step == 1):  #最初は日本語音節のみの単語辞書を使用(step==1が最初)
      ...
      p = os.popen( Juliusfolder + binfolder + " -C " + Juliusfolder + "syllable.jconf -C " + Juliusfolder + "am-gmm.jconf -v " + datafolder + trialname + "/" + str(step-1) + "/WD.htkdic -demo -filelist "+ datafolder + trialname + "/tmp/" + trialname + "/list.txt -lattice" ) #元設定-n 5 # -gram type -n 5-charconv UTF-8 SJIS  -confnet
