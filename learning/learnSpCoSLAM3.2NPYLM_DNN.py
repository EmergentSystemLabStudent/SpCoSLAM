...
# Akira Taniguchi 2017/01/18-2017/02/02-2017/02/15-2017/02/21-2017/07/17
##############################################
...
def ReadWordData(step, filename, particle):
      ...
      Otb_BOW = [ [0 for i in xrange(len(W_list))] for n in xrange(int(N/NbestNum)) ]
...
def WordDictionaryUpdate(step, filename, W_list):
  ...
  TANGO = []
  ##単語辞書の読み込み
  for line in open('./lang_m/' + lang_init_DNN, 'r'):
      itemList = line[:-1].split('	')
      LIST = LIST + [line]
      for j in xrange(len(itemList)):
...
if __name__ == '__main__':
    ...
    for i in xrange(R):
      ...
      latticelm_CMD = "latticelm -input text -prefix " + filename + "/out_gmm/" + str(i) + "_ -burnin " + str(burnin) + " -samps " + str(samps) + " -samprate " + str(samprate) + " -knownn " + str(knownn) + " -unkn " + str(unkn) + " -annealsteps " + str(annealsteps) + " -anneallength " + str(anneallength) + " " + filename + "/fst_gmm/sentences" + str(NbestNum) + ".char"
