...
#c-od-i-n-:-ut-f---8
#!/usr/bin/env python
#時系列データのグラフ描画プログラム、各評価値のstep推移をグラフ化する。（evaluate2_MI用）
#パーティクル平均の値を取得する
...
data1 ='SpCoA'
HYOUKA = ['NMIc','NMIi',"The number of spatial concepts","The number of position distributions","The number of segmented words","PAR","PARw","ARIc","ARIi","PRR"]
datasetNUM = 0
...
hs = raw_input("Evaluation? [NMIc(0),NMIi(1),L(2),K(3),SEG(4),PARwMI(5),PARw(6),ARIc(7),ARIi(8),PRR(9)]> ")
h = int(hs)
...
for d in xrange(len(DATA)):
  ...
  for s in range(N):
    ...
    #if (d == 0 or 2):
    #  data_name = 'TAMD2_sig'
    for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_meanEvaluation2MI.csv', 'r'):
      ...
      if (i != 0) and (itemList[0] != '') and (i <= step):
        ...
        PARw_M[d*N + (i-1)*len(DATA)*N + s] = float(itemList[5])
    ...
    i = 1
    for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_meanEvaluationPRR_MI.csv', 'r'): #_meanEvaluationPRR2.csv
      itemList = line[:-1].split(',')
    ...
    i = 0
    for line in open(datafolder + trialname + str(s+1).zfill(3) + '/' + trialname + str(s+1).zfill(3) +'_meanEvaluationSEG.csv', 'r'):
      itemList = line[:-1].split(',')
...
H_M = [NMIc_M,NMIi_M,EL_M,EK_M,ESEG_M,PARw_M,PARs_M,ARIc_M,ARIi_M,PRR_M]
#h = 0
...
if (1):
  ...
  #fig = AAA.get_figure()
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2_MI.eps', dpi=300)#, transparent=True
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2_MI.pdf', dpi=300)#, transparent=True
  plt.savefig(datafolder + '/Evaluation/' + trialname + '_' + data_num1 + '_' + data_num2 +  '_' + hyouka + '2_MI.png', dpi=300)#, transparent=True
  #h = h+1
